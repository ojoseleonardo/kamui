...
- Python 3.10+
- Dependências: em `core/`, `uv sync` ou `pip install -e .`
...
| GET | `/folder/videos` | Lista vídeos locais | — | Inclui `uploaded` por histórico |
| GET | `/events` | Timeline | `limit`, `offset`, `type`, `since`, `until` | |
| GET | `/events/stats` | Contagens por tipo | — | |
| DELETE | `/events` | Apaga todos os eventos | — | |
| GET | `/dashboard/summary` | Métricas dashboard | — | Uploads, pendentes, série 7 dias + views (Analytics) |
| POST | `/uploads/manual` | Upload por caminho | `{"path"}` | Ficheiro dentro da pasta monitorada |

## OAuth / scopes

- Credencial tipo **aplicação para computador**.
- Scopes usados: `youtube.upload`, `youtube.readonly`, `yt-analytics.readonly`.
- Ao adicionar scopes, é necessário **voltar a autorizar** em Configurações (novo token).
